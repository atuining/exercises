...
subString start end str
    ...
    | otherwise = take (e - s + 1) (drop s str)
        where
            e = max end 0
            s = max start 0
...
lowerAndGreater n list = show n ++ " is greater than " ++ show lower ++ " elements and lower than " ++ show greater ++ " elements"
    where
        [lower, greater] = fun n list [0,0]
            where
                fun :: Int -> [Int] -> [Int] -> [Int]
                fun val og t
                    | null og = t
                    | head og > val = fun val (tail og) [head t, 1 + last t]
                    | head og < val = fun val (tail og) [1 + head t, last t]
                    | otherwise = fun val (tail og) t

anotherLG :: Int -> [Int] -> String
anotherLG n list = show n ++ " is greater than " ++ show (length $ filter (< n) list) ++ " elements and lower than " ++ show (length $ filter (> n) list) ++ " elements"
